...
function buildNotes(primaryDx, answers, specialty = 'general practice') {
  const patientId = Session.id;
  const date = new Date().toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' });
  const qa = answers.filter(a => a.text && a.text !== '[Not answered]');
  return `
<strong>CUE PRE-CONSULTATION NOTE</strong> &nbsp;<span class="badge badge-teal" style="font-size:.7rem">BioMistral-7B · PubMed-trained</span><br/>
<strong>Patient ID:</strong> <span class="font-mono">${patientId}</span> &nbsp;·&nbsp; <strong>Date:</strong> ${date} &nbsp;·&nbsp; <strong>Specialty:</strong> ${specialty}<br/>
<span style="font-size:.75rem;color:var(--clr-slate-light)">Generated by BioMistral-7B (Apache 2.0, fine-tuned on PubMed Central) + Whisper large-v3 (MIT)</span>
<br/><br/>
<strong>S — SUBJECTIVE</strong><br/>
<em>Chief complaint:</em> ${qa[0]?.text || 'Not provided.'}<br/>
<em>History of presenting complaint:</em><br/>
${qa.slice(1).map(a => `<span style="color:var(--clr-slate-light);font-size:.82rem">${a.cat}:</span> ${a.text}`).join('<br/>')}<br/>
<br/>
<strong>O — OBJECTIVE</strong><br/>
Vitals: <em>Not yet assessed — to be completed at consultation</em><br/>
Examination findings: <em>Pending physical examination by clinician</em><br/>
<br/>
<strong>A — ASSESSMENT (AI impression)</strong><br/>
Primary: <strong>${primaryDx}</strong><br/>
<span style="color:var(--clr-warning);font-size:.82rem">⚠️ This AI-generated assessment is a clinical decision-support aid only. It must be reviewed, confirmed, or overridden by the attending physician before any treatment is initiated.</span><br/>
<br/>
<strong>P — PLAN (suggested — physician must confirm)</strong><br/>
• Confirm diagnosis by history and physical examination<br/>
• Order investigations as indicated by clinical findings<br/>
• Review and action any red-flag items flagged above<br/>
• Discuss management options and follow-up plan with patient
  `.trim();
}
